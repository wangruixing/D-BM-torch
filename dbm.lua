...
function create(dataset, labels, batch_size, layers,fantasy_count, learning_rate)
  self.dataset = dataset
  self.labels = torch.tensor({})
  self.datapts = dataset:size(1)
  self.batch_size = 50
  self.features = dataset:size(2)
  self.fantasy_count = 100
  self.learning_rate = 0.0001
  self.layers = {10,2}
  self.layers.insert(layers, getn(layers),1
                                          -- 'size': self.features,
...
function predict_probs(test, prop_uncertainty, omit_layers)
  prop_uncertainty = False
  omit_layers = 0
  out = test
...
function prob_given_vis(W, vs, bias, double)
  double  = false
...
function prob_given_out(W, hs, bias, double)
  double = false
...
function gibbs_update(gibbs_iterations, layers)
  gibbs_iterations = 10
  layers = nil
  if layers == nil then layers = getn(self.layers)
  end
  for j= 0, gibbs_iteration,1 do
    for i= 1, layers, 1 do
      double = i%2
      active = self.layers[i-1]['fantasy']
      bias = self.layers[i]['W']
      W = self.layers[i]['W']
      self.layers[i]['fantasy'] = self.sample(self.prob_given_vis)
--(W,active,bias,double)
    end
...
--- BP algorithm
function train_backprop(train_iteration, weight, layers)
  train_iteration = 100
  weight = 1
  layers = nil
  for iter= 0,train_iteraions,1 do
    ...
    self.backprop_step(self.dataset, self.labels, rate*weight, layers)
  end
end
--- gradient descent
function backprop_step(data, labels,rate, momentum_decay, train_layers)
  momentum_decay = 0
  train_layers = nil
  min = 0
  ...
  --Actuals
    for iter= layers-1, layer, -1 do
      ...
      prior_act = self.predict_probs(data, layers-iter)
...
---Training
function train_unsupervised(layer,train_iterations, gibbs_iterations)
  ...
  for iter= 0,train_iteration,1 do
    ...
    previous = torch.round(self.predict_probs(data,  layers-layer))
